...
This repository is released to make the Graph-attention Temporal Adaptor (GTA) independently checkable during review. It currently contains a minimal implementation of the CLIP baseline (global features only) + the GTA module, with the central GTA equations (Eqs. 10 and 11) implemented in `temporal_module.py` (Lines 71-77).
